...
export class CreateSubSubCategoryComponent implements OnInit {
  ...
  getList() {
    let request = { id: this.details.catid }
    this.userService.getSubCategoryByCatId(request)
      .pipe(take(1))
      .subscribe(res => {
        const { result } = res;
        this.listArr = result;
      ...
      this.userService.editSubCategory(fd)
        ...
        .subscribe(res => {
          if (res.code) {
            ...
            this.router.navigate(['/managecategory/getsubsubcategory'], { queryParams: { id: this.f.categoryId.value } });
          } else {
