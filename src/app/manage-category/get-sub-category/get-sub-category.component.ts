...
export class GetSubCategoryComponent implements OnInit {
  ...
  listHeaders: any = ['Image', 'Sub-Category Name', 'Total sub-sub category', 'Created on',
    'Action', 'Status'];
  listArr: any = [];
  listArrbackup: any = [];
  ...
  getList(params) {
    ...
    //   id:
    // }
    this.userService.getSubCategoryByCatId(params)
      .pipe(take(1))
      .subscribe(res => {
        const { result } = res;
        this.listArr = result;
  ...
  edit(id) {
    ...
    this.router.navigate(['/managecategory/editSubCategory'], { queryParams: { id: JSON.stringify(id) } });
  }
